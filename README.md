...
DESCRIPTION = ""
CREDITS = "@xvimp"
DISPLAY_CREDITS = "@xvimp"
UUID = "741dfd61-b890-4af7-91bf-021cbe421b66"
SETTINGS_PAGE = False

CREATOR_URL = "https://t.me/xvimp"
DISPLAY_CREATOR_URL = "https://t.me/xvimp"
CHANNEL_URL = "https://t.me/cloudecode"
...
CB_INFO = "at2:info"
CB_INSTRUCTION = "at2:instruction"
CB_INSTRUCTION_PAGE = "at2:instruction:p:"
...
def _home_text() -> str:
    return (
        ...
        f"👤 <b>Автор:</b> <a href=\"{DISPLAY_CREATOR_URL}\">{_h(DISPLAY_CREDITS)}</a>\n\n"
...
def _home_keyboard() -> K:
    ...
    keyboard.row(B("🗑 Удалить", callback_data=CB_DELETE_ASK))
...
def _instruction_pages() -> List[str]:
    """Встроенная инструкция по использованию плагина Auto Ticket."""
    return [
        (
            "<b>📖 Инструкция Auto Ticket</b>\n\n"
            "<b>Добро пожаловать!</b>\n\n"
            "Этот плагин автоматически отправляет тикеты в поддержку FunPay для проверки выполненных заказов.\n\n"
            "<b>Главные возможности:</b>\n"
            "• Автоматическое сканирование оплаченных заказов\n"
            "• Классификация заказов (обычные, проблемные)\n"
            "• Автоматическая отправка тикетов в поддержку\n"
            "• Настройка интервалов и правил\n"
            "• Поддержка AI классификации через io.net\n\n"
            "<b>Страница 1/5</b>"
        ),
        (
            "<b>📖 Авторизация</b>\n\n"
            "<b>Подключение к FunPay:</b>\n\n"
            "Плагин использует PHPSESSID для доступа к поддержке FunPay. Вы можете выбрать:\n\n"
            "1️⃣ <b>Автоматическое получение (рекомендуется)</b>\n"
            "При запуске Cardinal плагин автоматически получит PHPSESSID из вашего golden_key.\n\n"
            "2️⃣ <b>Ручной ввод</b>\n"
            "Введите PHPSESSID вручную в настройках авторизации.\n\n"
            "<i>Для первого запуска убедитесь, что golden_key правильно установлен в Cardinal.</i>\n\n"
            "<b>Страница 2/5</b>"
        ),
        (
            "<b>⚙️ Основные настройки</b>\n\n"
            "<b>Режимы классификации заказов:</b>\n\n"
            "🔵 <b>ОДИН СПИСОК</b> - все заказы в одном сообщении\n\n"
            "🟢 <b>ЛОКАЛЬНЫЕ ПРАВИЛА</b> - разделяет заказы по словам-признакам (спор, ошибка, возврат и т.д.)\n\n"
            "🟠 <b>IO.NET AI</b> - использует нейросеть для классификации\n\n"
            "<b>Интервалы:</b>\n"
            "• <b>Сканирование</b> - как часто проверять новые заказы\n"
            "• <b>Отправка</b> - как часто отправлять тикеты\n"
            "• <b>Возраст заказа</b> - минимальный возраст для включения в тикет\n"
            "• <b>Заказов в тикете</b> - сколько заказов в одном тикете (макс. 650)\n\n"
            "<b>Страница 3/5</b>"
        ),
        (
            "<b>📝 Текстовые шаблоны</b>\n\n"
            "<b>Настройка текстов тикетов:</b>\n\n"
            "Вы можете отдельно настроить тексты для разных режимов классификации.\n\n"
            "<b>Доступные переменные:</b>\n"
            "• <code>{orders}</code> - список заказов\n"
            "• <code>{username}</code> - ваш юзернейм\n"
            "• <code>{count}</code> - количество заказов\n"
            "• <code>{classification}</code> - тип заказа\n\n"
            "<b>Пример:</b>\n"
            "<i>Здравствуйте! Прошу подтвердить выполнение заказов: {orders}</i>\n\n"
            "<b>Страница 4/5</b>"
        ),
        (
            "<b>🚀 Использование и советы</b>\n\n"
            "<b>Как начать работу:</b>\n\n"
            "1. Перейдите в «Авторизация» и настройте подключение к FunPay\n"
            "2. Отредактируйте тексты тикетов в «Тексты»\n"
            "3. Установите интервалы сканирования в «Интервалы»\n"
            "4. Нажмите кнопку включения плагина\n\n"
            "<b>Полезные функции:</b>\n"
            "• Сканировать заказы вручную в «Дополнительно»\n"
            "• Просмотреть логи в «Обслуживание»\n"
            "• Обновить плагин локально через загрузку файла\n"
...
def _info_text() -> str:
    return (
        "<b>ℹ️ Информация</b>\n\n"
        "Здесь находятся ссылки на разработчика и официальный канал.\n\n"
        "• Telegram разработчика - связь с автором\n"
        "• Официальный канал - новости и обновления"
    )

def _info_keyboard() -> K:
    keyboard = K()
    keyboard.row(B("👤 Telegram автора", url=CREATOR_URL), B("📢 Канал", url=CHANNEL_URL))
    keyboard.row(B("📖 Инструкция", callback_data="at2:instruction:p:0"))
    keyboard.row(B("◀️ Назад", callback_data=CB_HOME))
    return keyboard
...
def _open_instruction(bot: Any, call: Any, page: int = 0) -> None:
    pages = _instruction_pages()
    ...
    nav: List[B] = []
    if page > 0:
        nav.append(B("⬅️", callback_data=CB_INSTRUCTION_PAGE + str(page - 1)))
    if page < max_page:
        nav.append(B("➡️", callback_data=CB_INSTRUCTION_PAGE + str(page + 1)))
    if nav:
        keyboard.row(*nav)
    keyboard.row(B("◀️ Назад", callback_data=CB_INFO))
    
    _safe_edit(bot, call, pages[page], keyboard)
    _answer(bot, call)
...
def init_cardinal(cardinal: Any) -> None:
    ...
    tg.cbq_handler(lambda call: _open_instruction(bot, call), func=lambda call: call.data == CB_INSTRUCTION)
    tg.cbq_handler(lambda call: _open_instruction(bot, call, int(call.data[len(CB_INSTRUCTION_PAGE):])), func=lambda call: call.data.startswith(CB_INSTRUCTION_PAGE))
